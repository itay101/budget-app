...
/**
 * Updates one or more fields of a transaction. Each editable cell in the
 * account's transaction table commits independently — only the fields
 * present in `formData` are touched, everything else is left as-is.
 *
...
export async function updateTransaction(formData: FormData) {
  ...
  const transaction = await prisma.transaction.findUniqueOrThrow({
    ...
    select: { accountId: true, amount: true, account: { select: { budgetId: true } } },
  });
  ...
  if (formData.has("payeeName")) {
    ...
    if (!payeeName) {
      ...
      data.payeeId = existing
        ? existing.id
        : (await prisma.payee.create({ data: { budgetId, name: payeeName } })).id;
    }
  }
